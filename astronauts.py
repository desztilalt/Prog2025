...
def iss_talalkozo():
    ...
    for szerep in szerepkorok:
        print(f"\t{szerep}:")
        for i in range(len(astronauts)):
            if astronauts[i][4] == "ISS" and astronauts[i][1] == szerep:
...
def pilota_tav():
    ...
    print(f"    {"Összes táv:":16} {pilota} millió km")
...
def hold_rang():
    print("4. feladat: A Holdra utazók rangjai\n")
    print(f"\t{'Becenév':<12} {'Rang'}")
    print("\t" + "-" * 22)
    for i in range(len(astronauts)):
        if astronauts[i][4] == "Hold":
            if astronauts[i][2] <= 10:
                rang = "*"
            elif astronauts[i][2] <= 20:
                rang = "**"
            else:
                rang = "***"
            print(f"\t{astronauts[i][0]:<12} {rang}")
